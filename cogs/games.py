...
class Games(commands.Cog):
    ...
    async def dice(self, ctx: commands.Context):
        ...
        await msg.edit(content='**Rolling..**')
        await msg.edit(content='**Rolling...**')
        await msg.edit(content=f'**You rolled a {random.randint(1, 6)}! :game_die:**',
    ...
    async def coinflip(self, ctx: commands.Context):
        ...
        await msg.edit(content='**Flipping..**')
        await msg.edit(content='**Flipping...**')
        await msg.edit(content=f'**You flipped a {random.choice(["Heads", "Tails"])}! :coin:**',
    ...
    async def truth(self, ctx: commands.Context, rating: str = None):
        ...
        embed.set_author(name='Truth', icon_url=self.bot.user.avatar.url)
    ...
    async def dare(self, ctx: commands.Context, rating: str = None):
        ...
        embed = discord.Embed(title=f'{data["question"]}', colour=discord.Colour.random())
        embed.set_footer(text=f'Type: {data["type"]} | Rating: {data["rating"].upper()} | ID: {data["id"]}')
        embed.set_author(name='Dare', icon_url=self.bot.user.avatar.url)
        await ctx.send(embed=embed, view=ui.TruthOrDareView(ctx=ctx, timeout=None))
    ...
    async def truthordare(self, ctx: commands.Context, rating: str = None, truthordare: str = None):
        ...
        embed = discord.Embed(title=f'{data["question"]}', colour=discord.Colour.random())
        embed.set_footer(text=f'Type: {data["type"]} | Rating: {data["rating"].upper()} | ID: {data["id"]}')
        embed.set_author(name='Truth or Dare', icon_url=self.bot.user.avatar.url)
        await ctx.send(embed=embed, view=ui.TruthOrDareView(ctx=ctx, timeout=None))
    ...
    async def wouldyourather(self, ctx: commands.Context, rating: str = None):
        ...
        embed = discord.Embed(title=f'{data["question"]}', colour=discord.Colour.random())
        embed.set_footer(text=f'Type: {data["type"]} | Rating: {data["rating"].upper()} | ID: {data["id"]}')
        embed.set_author(name='Would You Rather', icon_url=self.bot.user.avatar.url)
        await ctx.send(embed=embed, view=ui.TruthOrDareView(ctx=ctx, timeout=None))
    ...
    async def neverhaveiever(self, ctx: commands.Context, rating: str = None):
        ...
        embed = discord.Embed(title=f'{data["question"]}', colour=discord.Colour.random())
        embed.set_footer(text=f'Type: {data["type"]} | Rating: {data["rating"].upper()} | ID: {data["id"]}')
        embed.set_author(name='Never Have I Ever', icon_url=self.bot.user.avatar.url)
        await ctx.send(embed=embed, view=ui.TruthOrDareView(ctx=ctx, timeout=None))
    ...
    async def paranoia(self, ctx: commands.Context, rating: str = None):
        ...
        embed = discord.Embed(title=f'{data["question"]}', colour=discord.Colour.random())
        embed.set_footer(text=f'Type: {data["type"]} | Rating: {data["rating"].upper()} | ID: {data["id"]}')
        embed.set_author(name='Paranoia', icon_url=self.bot.user.avatar.url)
        await ctx.send(embed=embed, view=ui.TruthOrDareView(ctx=ctx, timeout=None))
    ...
    async def guessthenumber(self, ctx: commands.Context, limit: int = 100):
        ...
        while True:
            try:
                guess = await self.bot.wait_for('message', check=lambda m: m.author.id == ctx.author.id and m.channel.id == ctx.channel.id,
                                                timeout=60)  # type: discord.Message
    ...
    async def tictactoe_error(self, ctx: commands.Context, error: commands.CommandError):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            await send_error_embed(ctx,
                                   description=f'You need to specify a player to play against!\n\nProper Usage: `{self.bot.get_command("tictactoe").usage}`')
        elif isinstance(error, commands.BadArgument):
            await send_error_embed(ctx,
                                   description=f'Mention a valid player to play against!\n\nProper Usage: `{self.bot.get_command("tictactoe").usage}`')
        else:
            await send_error_embed(ctx,
                                   description='An error occurred while running the tictactoe command! The owner has been notified.')
            await inform_owner(self.bot, error)

    @commands.command(name='riddle', description='Play a competetive multiplayer riddle game',
                      usage='riddle <question_limit>')
    ...
    async def riddle(self, ctx: commands.Context, question_limit: int = 10):
        ...
        async with ctx.typing():
            ...
            questions = [question['question'].replace('&quot;', '"').replace('&#039;', "'") for question in
                         res['results']]
            answers = [question['correct_answer'].replace('&quot;', '"').replace('&#039;', "'") for question in
                       res['results']]

            embed = discord.Embed(title=questions[0], colour=0x0c1d4b).set_footer(
                text=f'Question 1 out of {question_limit}')
    ...
    async def riddle_error(self, ctx: commands.Context, error: commands.CommandError):
        ...
        if isinstance(error, commands.BadArgument):
            ...
            await send_error_embed(ctx,
                                   description=f'Command is on cooldown! Retry after {error.retry_after:.2f} seconds.')
        else:
            await send_error_embed(ctx,
